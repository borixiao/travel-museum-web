...
- ✅ **Firestore/Storage security rules verified** — the live rules aren't checked into this repo (no `firebase.json`/`*.rules` files exist; they're managed only in the Firebase Console for project `idm2526-summer-project`), so they were audited by hand against every actual read/write call in the codebase rather than by reading a tracked rules file:
  ...
  - ⚠️ **New `collections/{collectionId}` collection (added this pass for §4.2 "My Collections") still needs its matching owner-only rule added by hand in the Firebase Console** — mirroring the `items` rule above (owner-only read/create/update/delete via `userId`) — before it's safe to rely on for real users; not yet added since rules aren't tracked in this repo.
