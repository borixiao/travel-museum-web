...
- ✅ **"My Collections"** — user-created named groupings, separate from the single flat `items` list. New `collections` Firestore collection + `src/services/collections.ts` (`getCollections`/`createCollection`/`deleteCollection`); `Item` gained an optional `collectionId`. `HomePage.tsx` shows a tab row above the grid (All Items / Uncategorized / each collection) with inline "+ New Collection" create and a delete (✕) button on the active tab; deleting a collection only clears `collectionId` on its member items (reassigning them to Uncategorized) rather than deleting the items — it's a label, not a container. A "Collection" `<select>` in the item detail view (calling new `moveItemToCollection()`) lets an existing item be filed/refiled at any time. `UploadPage.tsx` also gained the same picker (with inline create) so a new item can be filed into a collection right at creation time, passed through `saveItem`'s new optional `collectionId` param. **Needs a manual Firestore security rule** for the new `collections` collection (owner-only read/write, mirroring the existing `items`/`moodboards`/`users` rules) — not tracked in this repo, must be added in the Firebase Console before this ships to real users.
