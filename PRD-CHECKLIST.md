...
- Note: avatar/display-name writes assume existing Firestore/Storage security rules already permit an authenticated user to write their own `users/{uid}` doc and `users/{uid}/avatar` Storage path (the same pattern as the original registration-time `setDoc` — not verified against the live rules in this pass, since they aren't checked into this repo)
