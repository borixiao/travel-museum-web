...
- ✅ **Custom background image** (beyond PRD, extends the color picker above) — "Upload background image" / "Change image" / "Remove image" controls next to the color picker; takes priority over `backgroundColor` when set. Uploaded to `moodboards/{userId}/background` (one moodboard per user, so no per-board keying needed — mirrors the avatar path pattern) via new `setMoodboardBackgroundImage`/`clearMoodboardBackgroundImage` in `services/moodboard.ts`, written to a new `Moodboard.backgroundImageUrl` field. **Required a new Storage rule** (`moodboards/{userId}/{allPaths=**}`, owner-only write) that didn't previously exist for this collection — added manually in the Firebase Console, not tracked in this repo (see §4.9's rules note). `MoodboardCanvas.tsx` renders it as `background-image: cover`; both the editor and the public `/m/:id` viewer read it. User-tested live and confirmed working before merging
